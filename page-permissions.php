<?php
	...
	if (@$_POST['grantrevoke']) {
		...
		if ($_POST['operation']=='grant')
			$success=no_displayed_error_result($permissiontxid, gamechain('grantfrom',
				$_POST['from'], $_POST['to'], implode(',', $permissions)));

		elseif ($_POST['operation']=='revoke')
			$success=no_displayed_error_result($permissiontxid, gamechain('revokefrom',
				$_POST['from'], $_POST['to'], implode(',', $permissions)));
	...
	if (no_displayed_error_result($getaddresses, gamechain('getaddresses', true))) {
		...
		if (no_displayed_error_result($listpermissions,
			gamechain('listpermissions', 'admin,activate', implode(',', array_get_column($getaddresses, 'address')))
		))
...
<?php
	if (no_displayed_error_result($listpermissions, gamechain('listpermissions'))) {
