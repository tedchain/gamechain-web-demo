<?php
	if (@$_POST['getnewaddress'])
		no_displayed_error_result($getnewaddress, gamechain('getnewaddress'));
?>
...
<?php
	if (no_displayed_error_result($peerinfo, gamechain('getpeerinfo'))) {
?>
...
<?php
	if (no_displayed_error_result($getaddresses, gamechain('getaddresses', true))) {
		...
		if (no_displayed_error_result($listpermissions,
			gamechain('listpermissions', 'all', implode(',', array_keys($addressmine)))
		))
			foreach ($listpermissions as $listpermission)
				$addresspermissions[$listpermission['address']][$listpermission['type']]=true;
		
		no_displayed_error_result($getmultibalances, gamechain('getmultibalances', array(), array(), 0, true));
