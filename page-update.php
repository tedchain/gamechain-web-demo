<?php
	...
	$max_upload_size=gamechain_max_data_size()-512; // take off space for file name and mime type

	$success=false; // set default value

	if (@$_POST['updateasset']) {
		if (!no_displayed_error_result($listassets, gamechain('listassets', $_POST['issuetxid'], true)))
			return;
		...
		$success=no_displayed_error_result($issuetxid, gamechain('createrawsendfrom', $from, $addresses, $datas, 'send'));
...
<?php
	if (no_displayed_error_result($listassets, gamechain('listassets', '*', true))) {
