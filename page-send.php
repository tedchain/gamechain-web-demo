<?php
	...
	if (@$_POST['sendasset']) {
		if (strlen($_POST['metadata']))
			$success=no_displayed_error_result($sendtxid, gamechain('sendwithmetadatafrom',
				$_POST['from'], $_POST['to'], array($_POST['asset'] => floatval($_POST['qty'])), bin2hex($_POST['metadata'])));
		else
			$success=no_displayed_error_result($sendtxid, gamechain('sendassetfrom',
...
<?php
	...
	if (no_displayed_error_result($getaddresses, gamechain('getaddresses', true))) {
		...
		if (no_displayed_error_result($listpermissions, gamechain('listpermissions', 'receive')))
