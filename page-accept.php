<?php
	if (@$_POST['unlockoutputs'])
		if (no_displayed_error_result($result, gamechain('lockunspent', true)))
	...
	if (@$_POST['decodeoffer'] || @$_POST['completeoffer']) {
		if (no_displayed_error_result($decoded, gamechain('decoderawexchange', $_POST['hex']))) {

			if (@$_POST['completeoffer']) {
				if (no_displayed_error_result($prepare, gamechain('preparelockunspentfrom', $_POST['from'], ask_offer_to_assets($decoded['ask'])))) {
						// output_success_text('Exchange successfully prepared using transaction '.$prepare['txid']);
			
					if (no_displayed_error_result($rawexchange, gamechain('appendrawexchange', $_POST['hex'],
						$prepare['txid'], $prepare['vout'], ask_offer_to_assets($decoded['offer'])))) {
			
						if (no_displayed_error_result($sendtxid, gamechain('sendrawtransaction', $rawexchange['hex'])))
...
<?php
	...
	$allassets=array();
	$haslocked=false;
	$getinfo=gamechain_getinfo();
	$labels=array();
	
	if (no_displayed_error_result($getaddresses, gamechain('getaddresses', true))) {
		if (no_displayed_error_result($listpermissions,
			gamechain('listpermissions', 'send', implode(',', array_get_column($getaddresses, 'address')))
		))
		...
		if (no_displayed_error_result($listassets, gamechain('listassets')))
			$allassets=array_get_column($listassets, 'name');

		foreach ($sendaddresses as $address) {
			if (no_displayed_error_result($allbalances, gamechain('getaddressbalances', $address, 0, true))) {
				
				if (count($allbalances)) {
					$assetunlocked=array();

					if (no_displayed_error_result($unlockedbalances, gamechain('getaddressbalances', $address, 0, false))) {
