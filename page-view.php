<?php
	...
	$labels=gamechain_labels();

	no_displayed_error_result($liststreams, gamechain('liststreams', '*', true));
	no_displayed_error_result($getinfo, gamechain('getinfo'));
	...
	foreach ($liststreams as $stream) {
		if (@$_POST['subscribe_'.$stream['createtxid']])
			if (no_displayed_error_result($result, gamechain('subscribe', $stream['createtxid']))) {
...
<?php
	...
	if (isset($viewstream)) {
		if (isset($_GET['key'])) {
			$success=no_displayed_error_result($items, gamechain('liststreamkeyitems', $viewstream['createtxid'], $_GET['key'], true, const_max_retrieve_items));
			$success=$success && no_displayed_error_result($keysinfo, gamechain('liststreamkeys', $viewstream['createtxid'], $_GET['key']));
			$countitems=$keysinfo[0]['items'];
			$suffix=' with key: '.$_GET['key'];
			
		} elseif (isset($_GET['publisher'])) {
			$success=no_displayed_error_result($items, gamechain('liststreampublisheritems', $viewstream['createtxid'], $_GET['publisher'], true, const_max_retrieve_items));
			$success=$success && no_displayed_error_result($publishersinfo, gamechain('liststreampublishers', $viewstream['createtxid'], $_GET['publisher']));
			$countitems=$publishersinfo[0]['items'];
			$suffix=' with publisher: '.$_GET['publisher'];
		
		} else {
			$success=no_displayed_error_result($items, gamechain('liststreamitems', $viewstream['createtxid'], true, const_max_retrieve_items));
...
?>
				...
				if (is_array($item['data'])) { // long data item
					if (no_displayed_error_result($txoutdata, gamechain('gettxoutdata', $item['data']['txid'], $item['data']['vout'], 1024))) // get prefix only for file name
