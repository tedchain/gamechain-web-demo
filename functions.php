<?php
	...
	function set_gamechain_chain($chain)
	{
		global $gamechain_chain;
		
		$gamechain_chain=$chain;
	}
	
	function gamechain($method) // other params read from func_get_args()
	{
		global $gamechain_chain;
		
		$args=func_get_args();
		
		return json_rpc_send($gamechain_chain['rpchost'], $gamechain_chain['rpcport'], $gamechain_chain['rpcuser'],
			$gamechain_chain['rpcpassword'], $method, array_slice($args, 1));
	}
	...
	function gamechain_getinfo()
	{
		global $gamechain_getinfo;
		
		if (!is_array($gamechain_getinfo))
			no_displayed_error_result($gamechain_getinfo, gamechain('getinfo'));
		
		return $gamechain_getinfo;
	}
	
	function gamechain_labels()
	{
		global $gamechain_labels;
		
		if (!is_array($gamechain_labels)) {
			if (no_displayed_error_result($items, gamechain('liststreampublishers', 'root', '*', true, 10000))) {
				$gamechain_labels=array();
				foreach ($items as $item)
					$gamechain_labels[$item['publisher']]=pack('H*', $item['last']['data']);
			}
		}
		
		return $gamechain_labels;
	}
	
	function gamechain_max_data_size()
	{
		global $gamechain_max_data_size;
		
		if (!isset($gamechain_max_data_size))
			if (no_displayed_error_result($params, gamechain('getblockchainparams')))
				$gamechain_max_data_size=min(
					$params['maximum-block-size']-80-320,
					$params['max-std-tx-size']-320,
					$params['max-std-op-return-size']
				);
		
		return $gamechain_max_data_size;
	}	
