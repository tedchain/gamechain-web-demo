<?php
	...
	if (@$_POST['issueasset']) {
		...
		$success=no_displayed_error_result($issuetxid, gamechain('createrawsendfrom', $_POST['from'], $addresses, $datas, 'send'));
	...
	if (no_displayed_error_result($getaddresses, gamechain('getaddresses', true))) {
		...
		if (no_displayed_error_result($listpermissions,
			gamechain('listpermissions', 'issue', implode(',', array_get_column($getaddresses, 'address')))
		))
